...
class UnauthenticatedJourneySpec extends BaseSpec {
  ...
  Feature("Journeys for completing the form and generating a PDF") {
    ...
    Scenario(
      ...
      child.ChildDateOfBirthPage(1).answer()
      child.ChildBiologicalSexPage(1).answer()
      child.ChildBirthRegistrationCountryPage(1).answerNorthernIreland()
      child.BirthCertificateHasNorthernIrishNumberPage(1).answerYes()
      child.ChildNorthernIrishBirthCertificateNumberPage(1).answer()
      child.AdoptingThroughLocalAuthorityPage(1).answerNo()
      child.ApplicantRelationshipToChildPage(1).answer()
